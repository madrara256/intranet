...
def index(request):
    ...
    context['client_addr'] = client
    return render(request,'website/layout/index.html', context)
...
def post_event(request):
    ...
    if request.POST and request.user:
        event = Event()
        if request.POST["event_title"] and request.POST["event_start_date"] and request.POST["event_end_date"] and request.POST["event_start_time"] and request.POST["event_end_time"] and request.POST["event_location"]:
            event.event_title = request.POST["event_title"]
            if request.POST["event_message"]:
...
def manage_data(request):
    ...
    try:
        ...
        news = News.objects.filter(created_by=request.user.id)
        branch_news = BranchNews.objects.filter(created_by=request.user.id)
    except:
        raise Http404
    context['events'] = events
    context['news'] = news
    context['branch_news'] = branch_news
    return render(request, 'website/layout/manage_data.html', context)
...
def edit_event(request, pk):
    ...
    if request.POST and request.user:
        ...
        event.event_end_time = request.POST['event_end_time']
        event.save()
        return redirect('website:manage_data')
    return render(request, 'website/layout/edit_event.html', context)
...
def edit_news(request, pk):
    context = {}
    general_news = News.objects.get(id=pk)
    context['news'] = general_news
    if request.POST and request.user:
        general_news = News.objects.get(id=pk)
        general_news.title = request.POST['post_title']
        general_news.message = request.POST['post_message']
        general_news.save()
        return redirect('website:manage_data')
    return render(request, 'website/layout/edit_general_news.html', context)
...
def edit_branch_news(request, pk):
    context = {}
    news = BranchNews.objects.get(id=pk)
    context['news'] = news
    if request.POST and request.user:
        news = BranchNews.objects.get(id=pk)
        news.title = request.POST['post_title']
        ...
        news.save()
        return redirect('website:manage_data')
    return render(request, 'website/layout/edit_branch_news.html', context)

def delete_branch_news(request, pk):
    general_news = BranchNews.objects.get(id=pk)
    general_news.delete()
    return redirect('website:manage_data')
